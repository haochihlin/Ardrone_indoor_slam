...
KeyFrameDisplay::KeyFrameDisplay()
{
	...
	//cloud2_BD_pub = n.advertise<sensor_msgs::PointCloud2>("pointcloud2_BD", 50);
	scale_para = 1.0;
	//---Jim---
...
//	minNearSupport = 9;
	...
	cloud.points.resize(height*width);
       ...
       /* sensor_msgs::PointCloud cloud_BD;
        cloud_BD.header.stamp = ros::Time(time);
        cloud_BD.header.frame_id = (ros::param::get("~PC2_frame_id", cloud_BD.header.frame_id)) ? cloud_BD.header.frame_id : "lsd_world";
	cloud_BD.points.resize(height*width);  */

	sensor_msgs::PointCloud2 cloud2; //for world frame
	//sensor_msgs::PointCloud2 cloud3; //for body frame
/*	cloud2.data.resize(height*width*12);	
	cloud2.header.stamp       = ros::Time::now();
	cloud2.header.frame_id    = "cloud_frame";
	cloud2.height             = 1;
	cloud2.width              = height*width;
	cloud2.is_bigendian       = true;
	cloud2.is_dense           = true;
	cloud2.point_step         = 12;
	cloud2.row_step           = height*width;
...
*/
	...
	for(int y=1;y<height-1;y++)
		for(int x=1;x<width-1;x++)
		{
			...
			cloud.points[total-1].x = scale * pt[0] * scale_para;
			cloud.points[total-1].y = scale * pt[1] * scale_para;
			cloud.points[total-1].z = scale * pt[2] * scale_para;
			
			//---For Body frame---
			/*cloud_BD.points[total-1].x = scale * (x*fxi + cxi) * depth * scale_para;
			cloud_BD.points[total-1].y = scale * (y*fyi + cyi) * depth * scale_para;
			cloud_BD.points[total-1].z = scale * depth * scale_para; */
			//---end---
	...
	//convertPointCloudToPointCloud2(cloud,cloud3);	// Pointcloud2 in BD
	//cloud2_BD_pub.publish(cloud3);
