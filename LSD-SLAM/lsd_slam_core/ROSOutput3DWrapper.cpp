...
ROSOutput3DWrapper::ROSOutput3DWrapper(int width, int height)
{
	...
	//---Jim---
	pose_tf_channel = nh_.resolveName("lsd_slam/pose_BD");
	pose_tf_publisher = nh_.advertise<geometry_msgs::PoseStamped>(pose_tf_channel,1);
...
void ROSOutput3DWrapper::publishTrackedFrame(Frame* kf)
{
	...
	//---Jim---	
	geometry_msgs::PoseStamped pMsg_tf;

	pMsg.pose.position.x = camToWorld.translation()[0] * scale_core;  //---Jim---
	pMsg.pose.position.y = camToWorld.translation()[1] * scale_core;  //---Jim---
	pMsg.pose.position.z = camToWorld.translation()[2] * scale_core;  //---Jim---
	...
	//---Jim---
	pMsg_tf.pose.position.x = -pMsg.pose.position.x;
	pMsg_tf.pose.position.y = -pMsg.pose.position.y;
	pMsg_tf.pose.position.z = -pMsg.pose.position.z;
	pMsg_tf.pose.orientation.x = -pMsg.pose.orientation.x;
	pMsg_tf.pose.orientation.y = -pMsg.pose.orientation.y;
	pMsg_tf.pose.orientation.z = -pMsg.pose.orientation.z;
	pMsg_tf.pose.orientation.w = pMsg.pose.orientation.w;	

	if (pMsg_tf.pose.orientation.w < 0)
	{
		pMsg_tf.pose.orientation.x *= -1;
		pMsg_tf.pose.orientation.y *= -1;
		pMsg_tf.pose.orientation.z *= -1;
		pMsg_tf.pose.orientation.w *= -1;
	}
	//---End---

	
	pMsg.header.stamp = ros::Time(kf->timestamp());
	pMsg.header.frame_id = (ros::param::get("~lsd_world_frame", pMsg.header.frame_id)) ? pMsg.header.frame_id : "lsd_world";
	pose_publisher.publish(pMsg);
	//---Jim---
	pMsg_tf.header.stamp = ros::Time(kf->timestamp());
	pMsg_tf.header.frame_id = (ros::param::get("~lsd_pose_frame", pMsg_tf.header.frame_id)) ? pMsg_tf.header.frame_id : "cloud_frame";
	pose_tf_publisher.publish(pMsg_tf);
	//---Jim---
	static tf::TransformBroadcaster br;
	tf::Transform transform;
	transform.setOrigin(tf::Vector3(pMsg.pose.position.x, pMsg.pose.position.y, pMsg.pose.position.z));
	transform.setRotation(tf::Quaternion( pMsg.pose.orientation.x, pMsg.pose.orientation.y, pMsg.pose.orientation.z, pMsg.pose.orientation.w));
	br.sendTransform(tf::StampedTransform(transform, ros::Time(kf->timestamp()), "lsd_world", "lsd_pose"));
}
