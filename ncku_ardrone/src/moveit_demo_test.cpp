...
ArdroneControl::ArdroneControl()
{
  ...
  lsd_PC2_sub = n.subscribe("/pointcloud2", 1, &ArdroneControl::lsd_PC2_callback, this);
...
void ArdroneControl::path_callback(const moveit_msgs::DisplayTrajectory msg)
{
	...
	while( i_next < point_size )
		...
		ROS_INFO("x:%f, y:%f, yaw:%f",path_x[i_next],path_y[i_next],path_w[i_next]);		
		i = i_next;		
		i_next++;
	    }
        }
...
int main(int argc, char **argv)
{
	...
	ardrone_control.Plan_request(5,5,1.57);
	ROS_INFO("1st finished");	

	sleep(2.0);
	ardrone_control.Plan_request(-5,-5,1.57);
	ROS_INFO("2nd finished");

	sleep(4.0);
	ardrone_control.Plan_request(5,-5,1.57);
	ROS_INFO("3rd finished");
